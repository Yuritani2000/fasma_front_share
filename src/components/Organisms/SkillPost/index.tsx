...
const SkillPost: React.FC<SkillPostProps> = (props) => {
  ...
  const categoryItem = [
    { label: 'カテゴリを選択' },
    ...new Array<Items>(mockCategories.length).fill({ label: '' }).map((item, index) => {
      return { label: mockCategories[index] };
    }),
  ] as Items[];
          ...
          <InputElement>
            <Input
              ...
              textOnChange={(value) => {
                ...
                setSkillPrice(parseInt(value));
              }}
              value={skillPrice.toString()}
              inputType={'text'}
            />
          </InputElement>
        </OneInputParent>
